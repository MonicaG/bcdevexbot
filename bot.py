"""
Calls the BC Developer Exchange API and tweets any issues added since the
...
class BCDevExBot(models.Base):
    ...
    def process(self):
        ...
        stored_issues = persistence.DataStore(
            self.config['file']['pickle_file'])
        seen_issues = stored_issues.get()
        issue_ids = []
        open_issues = models.BCDevExchangeIssues().get_opportunities()
        # There are two try blocks because:
        # Inner try block deals with any exceptions while tweeting.  This
        # allows for one issue to fail but allows for the next issue to be
        # processed.  Only the processed issue_ids will be stored.
        # The outer try block deals with the case where the response data
        # could not be processed  (i.e. a field is missing).  But, if any data
        # had been successfully tweeted, then we want to store those issue ids.
        try:
            for data in open_issues:
                ...
                try:
                    if not (issue_id in seen_issues):
                        ...
                        logger.info("Skipping {0}. Processed previously."
                                    .format(issue_id))
                except Exception:
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(description=(
        'Calls the BC Developer Exchange API and tweets any issues '
        'added since the last time this process ran.'))
    arg_parser.add_argument('config_file',
                            help='Contains the configuration needed to run '
                            'this bot')
