...
URL_LENGTH = 20
TWITTER_CONFIG = {'short_url_length': URL_LENGTH, 'short_url_length_https': URL_LENGTH}
...
def tweet(mock_tweepy):
    ...
    tweeter = models.Twitter(credentials, persistence.DataStore('/path/to/file'))
...
def test_status_creation_equals_max_status_length(mock_twitter_config, tweet):
    ...
    mock_twitter_config.return_value = TWITTER_CONFIG
    status = tweet._create_status(url,
                                  'Here is a title. We need enough characters to make a status exactly 140 characters '
                                  'long. 1234567890ab',
                                  'New Issue: '
                                  )
...
@patch('bcdevexbot.persistence.DataStore.get')
def test_status_creation_1_char_longer_max_status_length(mock_twitter_config, tweet):
    url = 'http://example.org/1'
    # see comment in above test on why the url length needs to be a certain length
    assert URL_LENGTH == len(url)
    mock_twitter_config.return_value = TWITTER_CONFIG
...
def test_multiple_calls_to_update_status(mock_twitter_config, tweet):
    ...
    tweet._api.update_status.assert_called_with("New issue: Title 2 example http://example2.org #BCDev")
    mock_twitter_config.assert_called_once_with()
...
def test_https_link(mock_twitter_config, tweet):
    ...
    tweet._api.update_status.assert_called_with("New issue: A title example https://example.org #BCDev")
    mock_twitter_config.assert_called_once_with()
